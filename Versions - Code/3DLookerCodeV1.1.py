...
def init_colors(WallColor):
    # Example: factors for distance shading
    init_pair_colors(WallColor, [0.9, 0.8, 0.65, 0.5, 0.3])
...
def draw_minimap(stdscr, game_map, player, offset_y, offset_x, scale=2):
    ...
    if player_screen_y < max_y and player_screen_x < max_x:
        try:
            stdscr.addstr(player_screen_y, player_screen_x, "@", curses.color_pair(2) | curses.A_BOLD)
        except curses.error:
            pass
...
def main(stdscr):
    ...
    WallColor = [235, 0, 109]  # wall color
    init_colors(WallColor)
    ...
    while True:
        ...
        for i in range(num_rays):
            ...
            if distance < 1.5:
                char = "█"
                charColor = curses.color_pair(1)  # RED - very near
            elif distance < 3:
                char = "▓"
                charColor = curses.color_pair(2)  # YELLOW - near
            elif distance < 5:
                char = "▒"
                charColor = curses.color_pair(3)  # GREEN - medium
            elif distance < 7:
                char = "▒"
                charColor = curses.color_pair(4)  # CYAN - far
            elif distance < 10:
                char = "░"
                charColor = curses.color_pair(5)  # BLUE - very far
            elif distance < 14:
                char = "."
                charColor = curses.color_pair(6)  # MAGENTA - ultra far
            else:
                char = " "
        ...
        if show_Info:
            info_msgs = [
                ...
                f"FPS: {int(1 / 0.016)}",
                f"Runtime: {RUNTIME:.1f} seconds ({minruntime} Minutes)",
                f"Map Size: {maze_width} x {maze_height}",
                f"Show Minimap: {'ON' if show_minimap else 'OFF'}",
                f"Main Wall Color = {WallColor} (RBG Color code)"
            ]
            infolengt = 0
    ...
    stdscr.addstr(1, 0, f"Runtime = {RUNTIME} ({minruntime} Minutes)", curses.color_pair(2))
    stdscr.refresh()
    time.sleep(3)
